...
export function TenantLoginScreen() {
  ...
  const fromPath = useMemo(() => location?.state?.from || '/tenant-dashboard', [location])
  ...
  const onVerifyOtp = async (values: OtpForm) => {
    try {
      ...
      if (res.data.data.refreshToken) setCookie('refresh_token', res.data.data.refreshToken)
              ...
              <Form {...phoneForm}>
                ...
                >
                  ...
                  <Button
                    type='submit'
                    disabled={sending}
                    className='h-10 w-full rounded-full bg-blue-600 text-sm font-medium hover:bg-blue-700'
                  >
                ...
                >
                  ...
                  <div className='flex items-center justify-center gap-2 pt-2'>
                    ...
                    <button
                      type='button'
                      onClick={() => {
                        const phoneWithoutPrefix = fullPhone.split(' ')[1] || ''
      ...
      <div className='order-2 hidden h-full w-full flex-col items-center justify-center bg-gradient-to-br from-blue-600 via-blue-700 to-slate-900 text-white lg:order-1 lg:flex lg:w-1/2'>
        <div className='flex flex-col items-center justify-center p-12'>
          ...
          <div className='mt-8 text-xs text-white/60'>
            Tenant Portal - View your PG details & pay rent
          </div>
        </div>
      </div>
    </div>
