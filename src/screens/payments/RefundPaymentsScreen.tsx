...
export function RefundPaymentsScreen() {
  ...
  const queryArgs = useMemo(() => {
    const params: GetRefundPaymentsParams & { page: number; limit: number } = {
      page,
      limit,
    }
    ...
    return params
  }, [computedDates.end_date, computedDates.start_date, limit, page, selectedMonth, selectedYear, statusFilter])
      ...
      <div className='mt-4 grid gap-3'>
        {isLoading ? (
          ...
          <div className='rounded-md border bg-card px-3 py-8 text-center'>
            ...
            <div className='mt-1 text-xs text-muted-foreground'>No refund payments found.</div>
          </div>
        ) : (
          items.map((p) => {
                  ...
                  <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
                    <div className='min-w-0'>
                      <div className='flex items-center gap-2'>
                        <div className='rounded-md bg-red-500/10 px-2 py-1 text-[10px] font-semibold text-red-600'>
                          💸 REFUND
                        </div>
                      ...
                      <div className='mt-0.5 text-[11px] text-muted-foreground'>
                        {where ? where : 'Room N/A · Bed N/A'}
                      </div>
                    </div>

                    <div className='text-right'>
                      <div className='text-base font-semibold text-red-600'>{formatMoney(p.amount_paid)}</div>
                    </div>
                  </div>

                  <div className='mt-3 rounded-md border bg-red-500/5 p-3 text-xs'>
